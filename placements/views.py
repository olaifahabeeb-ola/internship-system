...
def review_placement(request, pk):
    ...
    if request.method == 'POST' and form.is_valid():
        ...
        if decision == 'approved':
            ...
            send_notification(
                user=placement.submitted_by,
                message=f'Your placement "{placement.title}" was approved and is now live.',
                notification_type='placement',
                link=reverse('placements:supervisor_my_placements'),
            )
            messages.success(
                request,
            ...
            send_notification(
                user=placement.submitted_by,
                message=f'Your placement "{placement.title}" was rejected.',
                notification_type='placement',
                link=reverse('placements:supervisor_my_placements'),
            )
            messages.info(
...
def review_application(request, pk):
    ...
    if request.method == 'POST' and form.is_valid():
        ...
        send_notification(
            ...
            message=(
                f'Your application for "{application.placement.title}" at '
                f'{application.placement.company_name} was {decision}.'
            ),
            notification_type='application',
            link=reverse('placements:my_applications'),
        )
        ...
        if decision == 'accepted':
            ...
            for other in other_pending:
                ...
                send_notification(
                    user=other.student,
                    message=(
                        f'Your application for "{other.placement.title}" was '
                        f'automatically closed — you were placed elsewhere.'
                    ),
                    notification_type='application',
                    link=reverse('placements:my_applications'),
                )
